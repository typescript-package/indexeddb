...
import { rawSchemaDBwithConnection, rawSchemaDBwithParameters, Shop, shopSchemaDBWithConnection } from "./data";
...
const shopSchemaQueryWithData = IDBQuery.create<Shop>()({
  'store': {
    'person': {
      'add': {
        'value': { name: 'Alice' },
...
shopSchemaQueryWithData.store('person', {
})
...
idbQueryConstructorWithData.store('cart', {
  'add': {
